...
function applyDarkMode(dark) {
  ...
  if (btn) { btn.innerHTML = dark ? sunSVG : moonSVG; btn.title = dark ? '切換淺色模式' : '切換深色模式'; }
}

applyDarkMode(localStorage.getItem('wd-admin-dark') === '1');

document.getElementById('dark-mode-toggle')?.addEventListener('click', () => {
  const nowDark = !document.body.classList.contains('dark');
  localStorage.setItem('wd-admin-dark', nowDark ? '1' : '0');
